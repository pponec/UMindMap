...
- **Nothing to install.** Copy `docs/` onto any static host — or open it  straight from GitHub Pages, as above.
- **Nothing to learn.** If you can write a bullet list, you can use it.
- **No lock-in and no bloat.** Under 2 800 lines of vanilla JavaScript, zero  dependencies, Apache 2.0.

## Desktop first

UMind is built around a keyboard, and it shows.
On a phone — Android in particular — the on-screen keyboard opens as soon as you touch a node and covers a good part of the screen, which leaves little room for the map you are
editing.
Reading a map, folding branches and looking at the picture are fine on a small screen; typing into one is cramped.
Making the phone a comfortable
...
browser's **localStorage** under the project's name.
...
which needs the File System Access API and therefore Chromium; everywhere else the same button simply downloads the file.
Serve the app over `http` rather
...
> browser storage.
Sending that URL to somebody else opens *their* browser with
...
The welcome map is a preview that is never saved, your
...
**private** project from *your own* browser storage.
A name *ending* in
...
to somebody else and trust that they see the same map.
Pressing **Edit map**
forks a private, editable copy into their browser without ever touching the
published file.
Publishing one is just dropping a `.json` into `docs/data/`
...
| `src` value | Result |
|---|---|
| `https://example.com/pic.png` | Loads from that server.
Works anywhere. |
| `images/pic.png` (relative) | Resolved against the **page origin** (e.g.
`https://…github.io/UMind/images/pic.png`), so the image must be deployed alongside the app — never read from the visitor's disk. |
...
tab.
That is not a corner cut — it *is* the security model, and it is worth
...
- **Shared map files are read-only and locked to one folder.** A `?name.json`
  ...
  the server.
  A shared map opens as a read-only picture; **Edit map** forks a
...
- **Markdown is rendered, not executed.** The description renderer builds DOM
  ...
  construction — there is no `innerHTML` path for markup to slip in.
  The
...
read-only** maps.
Each link below opens the live app and renders that map as a
